...
namespace AoC2018
{
    class Program
    {
        ...
        public static bool SimulateBattle(int elvenPower = 3)
        {
            ...
            string[] linesInput = File.ReadAllLines(@"C:\puzzle15.txt");
        ...
        public static bool GetTargets(Unit soldier, Dictionary<Point, PrevPoint> dctPoints, out Point newPosition)
        {
            ...
            else
            {
                ...
                return true;
                /*
                var lastTarget = potentialTargets.OrderBy(r => r.Y).ThenBy(w => w.X).First();
                var dissectTarget = dctPoints[lastTarget];
                if (dissectTarget.cost == 1)
                {
                    newPosition = dctPoints.Single(r => r.Key == lastTarget).Key;
        ...
        /*
        public static bool CheckEndCondition()
        {
            if (lstUnits.All(r => r.faction == Faction.Elf) || lstUnits.All(r => r.faction == Faction.Goblin))
                return true;
            else
                return false;
        }*/
